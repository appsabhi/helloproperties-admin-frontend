/**
 * Comprehensive dataset of States and Union Territories of India with their respective Districts.
...
export const INDIA_LOCATION_DATA = {
  ...
  "Odisha": [
    ...
    "Ganjam", "Jajpur", "Jagatsinghpur", "Jharsuguda", "Kalahandi", 
...
export function getDistrictsForState(stateName) {
  if (!stateName || !INDIA_LOCATION_DATA[stateName]) {
    // If state is not selected, return all unique districts across India
    const allDistricts = new Set();
    Object.values(INDIA_LOCATION_DATA).forEach(list => {
      list.forEach(d => allDistricts.add(d));
    });
    return Array.from(allDistricts).sort();
  }
  return INDIA_LOCATION_DATA[stateName].slice().sort();
}

export function getStateForDistrict(districtName) {
  if (!districtName) return '';
  const dNorm = districtName.toLowerCase().trim();
  for (const [state, districts] of Object.entries(INDIA_LOCATION_DATA)) {
    if (districts.some(d => d.toLowerCase().trim() === dNorm)) {
      return state;
    }
  }
  return '';
